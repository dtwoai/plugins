...
| `oauth` | `grant_type`, plus either `issuer` (for DCR) or `client_id` + `client_secret` + `token_url`. Optional: `scopes` (omit it or set `[]` for providers that reject any `scope` parameter — the gateway then sends none; otherwise list the provider's documented scopes; on the DCR (`issuer`-only) path an omitted `scopes` registers the client with no scopes at all — the gateway's own DCR default does not apply), `authorization_url`, `redirect_uri`, `pkce_enabled`, `token_endpoint_auth_method`, `omit_resource`, `extra_authorize_params`, `scope_param_name`, `scope_separator`, `token_response_path`, `token_lifetime_seconds`, `oauth_quirks` |
| `cert` | `ca_cert` (PEM) |
