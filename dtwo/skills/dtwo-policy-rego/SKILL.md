...
- **`preferred_username` / `upn` / `unique_name`** *(Microsoft Entra / Azure AD)* — where the human-readable identifier lives on Entra tokens, and a common trap for policies written against Auth0. On **Entra v1** access tokens, `sub` is a per-application **pairwise pseudonym** — an opaque string that differs for every app registration — so it is *not* a portable identity; the email-shaped value lives in `upn` and `unique_name`. **Entra v2** access tokens instead surface `preferred_username`. A policy that keys on `sub` alone therefore behaves differently on Entra than on Auth0/Keycloak. When an identity check must work across IdPs, resolve in the order `email` → `preferred_username` → `upn` → `unique_name`, treating `sub` as a last-resort opaque key. Caveat: `preferred_username` / `upn` are *mutable* and not guaranteed stable over a user's lifetime — for a durable per-user key, prefer `sub` scoped by `iss`, or Entra's `oid` claim when the token carries it.
...
>
> - Comparing on `email`? Guard it — require `object.get(input.subject.claims, "email_verified", false) == true` in the same rule body. Treat only an explicit `true` as verified (a missing or falsy `email_verified` means *not* verified).
> - Otherwise key on `sub` (scoped by `iss`) or `upn`, which the IdP vouches for directly.
>
> Never compare `email` for an authorization decision without the `email_verified` guard.
...
- **Treating `input.subject.sub` as an email address:** For tokens issued by ContextForge itself, `sub` is the user's email. For external IdP tokens, `sub` is whatever the IdP put in the `sub` claim — typically an IdP-prefixed identifier like `google-apps|paul@dtwo.ai`, not a clean email. On **Microsoft Entra v1** tokens `sub` is worse still: a per-application **pairwise pseudonym** that is opaque *and* differs across apps, so it is useless as a shared identity — read `upn` / `unique_name` (v1) or `preferred_username` (v2) instead. If you need a stable user identifier, compare against the full `sub` value; if you need an email, prefer `input.subject.claims.email` (when the IdP emits it) and fall back to `sub` only when you've confirmed the IdP issues email-shaped subs.
